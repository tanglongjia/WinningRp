	...
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/label.html',
				name:thePlugins,
				editor:this,
				title: '标签',
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>标签: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'seal');
 * ```
 */
UE.plugins['seal'] = function () {
    var me = this,thePlugins = 'seal';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/seal.html',
                name:thePlugins,
                editor:this,
                title: '印章',
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        if ( /img/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
            var html = popup.formatHtml(
                '<nobr>印章: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
            if ( html ) {
                popup.getDom( 'content' ).innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor( popup.anchorEl );
...
UE.registerUI('button_leipi',function(editor,uiName){
	//console.log(uiName);button_leipi
...
UE.registerUI('button_myprint',function(editor,uiName){
    if(!this.options.toolleipi)
    {
        return false;
    }
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                leipiFormDesign.fnMyPrint();
            } catch ( e ) {
                alert('leipiFormDesign.fnMyPrint 打印异常');
            }
        }
    });
    //创建一个button

...
(function ($) {
    ...
    $.fn.bootstrapSelect.methods = {
        ...
        setData: function (target, datas) {
            return $(target).data('bootstrap.select').setData(datas);
        },
        /**
         * 根据id获取文本值
         */
     ...
     * 重新加载
     */
    BootstrapSelect.prototype.setData = function (datas) {
        var $this = this;
    ...
    BootstrapSelect.prototype.select = function (selItem, keyEvent) {
        ...
        if (unSelect) {
            ...
            options.onSelect.call(this, $selItem.data('jsonData')[$this.options.valueField], $selItem.data('jsonData'), $this);
        }
    };
    /**
     * 清空下拉节点
    ...
    function makeContAndShow(inputDownList, isReload) {
        ...
        for (var i = 0; i < data_.length; i++) {
            itemsHtml += '<div style="padding-left:17px;width:100%;">' + data_[i][options.textField] + '</div>';
        }
        $this.html(itemsHtml);
